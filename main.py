...
if config_params['multi_objective']:
    ...
    rnd = RNDUncertainty(400, env_params['states'][0][0], device)
    ...
    rnd = RNDUncertainty(400, env_params['states'][0][0], device)
    experiment = Experiment_SO(learner=learner, buffer=buffer, env=env, params=config_params, device=device, uncertainty=rnd)
experiment.run()
